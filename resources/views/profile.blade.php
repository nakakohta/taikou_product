...
.profile-info h1{ font-size:22px; font-weight:900; margin:0 0 4px; }
.profile-info p{ font-size:14px; opacity:.7; margin:0 0 10px; }
...
.file-wrap{ display:flex; align-items:center; gap:12px; padding:12px 14px; border-radius:16px; border:2px solid var(--border, #dbeafe); background: var(--bg, #f5faff); overflow:hidden; }
.file-input{ position:absolute; width:1px; height:1px; overflow:hidden; clip:rect(0 0 0 0); clip-path:inset(50%); }
.file-btn{ padding:8px 14px; border-radius:14px; background:#eaf3ff; border:2px solid var(--border, #dbeafe); font-size:14px; font-weight:800; cursor:pointer; white-space:nowrap; }
...
.panel{ background: var(--card, #fff); border:2px solid var(--border, #dbeafe); border-radius:22px; padding:20px; min-height:180px; }
...
.song-title{ font-weight:900; color: var(--text-blue, #3b82f6); text-decoration:none; }
.song-meta{ font-size:13px; opacity:.7; margin-top:4px; }
...
.fav-right{ display:flex; gap:10px; align-items:center; }
.badge-mini{
  padding:6px 10px;
  border-radius:999px;
  border:2px solid var(--border,#dbeafe);
      ...
      @if(($favoriteRanking ?? collect())->count())
        @foreach($favoriteRanking as $row)
          <div style="display:flex;justify-content:space-between;align-items:center;margin-top:14px;">
            ...
            <div style="opacity:.7;font-weight:900;">{{ $row['count'] }}回</div>
          </div>
